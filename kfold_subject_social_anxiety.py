...
def evaluate(reg_fix, reg_sac, X_fix_test, y_f_test, stim_f_test, sub_f_test, X_sac_test, y_s_test, stim_s_test, sub_s_test, fold, config):
    ...
    bool_mask = [ x==1 for x in sub_f_test ]
    print('ppred_fix ', np.mean(ppred_fix[bool_mask]) )
    print(key_fix, ppred_fix_comb)
...
def get_results_kfold(X_fix, ids_f, yf, stim_f, X_sac, ids_s, ys, stim_s, k, model, config):
    ...
    #print('sub_splits', sub_splits_gen)
    ...
    for fold, (train_index, test_index) in enumerate(kf.split(sub_ids)):
        print('train_index', sub_ids[train_index])
        print('test_index', sub_ids[test_index])

        bool_mask_fix = [x in sub_ids[train_index] for x in ids_f]
        bool_mask_sac = [x in sub_ids[train_index] for x in ids_s]

        print('\nFold ' + str(fold+1) + ' of ' + str(k))

        train_Xf = X_fix[bool_mask_fix]
        train_yf = yf[bool_mask_fix]
        train_stf = stim_f[bool_mask_fix]
        train_ids_f = ids_f[bool_mask_fix]

        train_Xs = X_sac[bool_mask_sac]
        train_ys = ys[bool_mask_sac]
        train_sts = stim_s[bool_mask_sac]
        train_ids_s = ids_s[bool_mask_sac]


        test_Xf = X_fix[np.logical_not(bool_mask_fix)]
        test_yf = yf[np.logical_not(bool_mask_fix)]
        test_stf = stim_f[np.logical_not(bool_mask_fix)]
        test_ids_f = ids_f[np.logical_not(bool_mask_fix)]

        print('train_ids_f ', set(train_ids_f))
        print('test_ids_f ', set(test_ids_f) )

        test_Xs = X_sac[np.logical_not(bool_mask_sac)]
        test_ys = ys[np.logical_not(bool_mask_sac)]
        test_sts = stim_s[np.logical_not(bool_mask_sac)]
        test_ids_s = ids_s[np.logical_not(bool_mask_sac)]

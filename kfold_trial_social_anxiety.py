...
def train_sklearn(X, y, model, fold, config, model_type='fix'):
    ...
    if type(pipe_reg[1]) == RandomForestRegressor: 
        with open('./results/kfold_trial/features_importance/RF_'+model_type+'_'+config+'_'+str(fold)+'.npy', 'wb') as f:
...
def normalize_data_and_train_gpr(train_X, train_y, test_X, fold, config, model_type):
    ...
    with open('./results/kfold_trial/features_importance/GPR_'+model_type+'_'+config+'_'+str(fold)+'.npy', 'wb') as f:
...
def evaluate(reg_fix, reg_sac, X_fix_test, y_f_test, stim_f_test, sub_f_test, X_sac_test, y_s_test, stim_s_test, sub_s_test, fold, config):
    ...
    if type(reg_fix) ==  GPy.models.SparseGPRegression:
        ...
        with open('./results/kfold_trial/datapoint_based/'+type(reg_fix).__name__+'_ppred_fix_'+config+'_'+str(fold)+'.npy', 'wb') as f:
            np.save(f, ppred_fix)
        with open('./results/kfold_trial/datapoint_based/'+type(reg_fix).__name__+'_ppred_fix_var_'+config+'_'+str(fold)+'.npy', 'wb') as f:
            np.save(f, ppred_fix_var)
        with open('./results/kfold_trial/datapoint_based/'+type(reg_fix).__name__+'_y_fix_'+config+'_'+str(fold)+'.npy', 'wb') as f:
            np.save(f, y_f_test)
    else:
        ppred_fix = reg_fix.predict(X_fix_test)
        #Saving datapoint based results
        with open('./results/kfold_trial/datapoint_based/'+type(reg_fix[1]).__name__+'_ppred_fix_'+config+'_'+str(fold)+'.npy', 'wb') as f:
            np.save(f, ppred_fix)
        with open('./results/kfold_trial/datapoint_based/'+type(reg_fix[1]).__name__+'_y_fix_'+config+'_'+str(fold)+'.npy', 'wb') as f:
            np.save(f, y_f_test)
    ...
    if type(reg_sac) ==  GPy.models.SparseGPRegression:
        ...
        #Saving datapoint based results
        with open('./results/kfold_trial/datapoint_based/'+type(reg_sac).__name__+'_ppred_sac_'+config+'_'+str(fold)+'.npy', 'wb') as f:
            np.save(f, ppred_sac)
        with open('./results/kfold_trial/datapoint_based/'+type(reg_sac).__name__+'_ppred_sac_var_'+config+'_'+str(fold)+'.npy', 'wb') as f:
            np.save(f, ppred_sac_var)
        with open('./results/kfold_trial/datapoint_based/'+type(reg_sac).__name__+'_y_sac_'+config+'_'+str(fold)+'.npy', 'wb') as f:
            np.save(f, y_s_test)
    else:
        ppred_sac = reg_sac.predict(X_sac_test) # Gaussian processes return both prediction and uncertainty
        #Saving datapoint based results
        with open('./results/kfold_trial/datapoint_based/'+type(reg_sac[1]).__name__+'_ppred_sac_'+config+'_'+str(fold)+'.npy', 'wb') as f:
            np.save(f, ppred_sac)
        with open('./results/kfold_trial/datapoint_based/'+type(reg_sac[1]).__name__+'_y_sac_'+config+'_'+str(fold)+'.npy', 'wb') as f:
            np.save(f, y_s_test)
    ...
    if type(reg_sac) ==  GPy.models.SparseGPRegression:
        ...
        with open('./results/kfold_trial/trial_based/'+type(reg_sac).__name__+'_y_pred_'+config+'_'+str(fold)+'.npy', 'wb') as f:
            np.save(f, y_pred)
        with open('./results/kfold_trial/trial_based/'+type(reg_sac).__name__+'_y_test_'+config+'_'+str(fold)+'.npy', 'wb') as f:
            np.save(f, y_test)
    else:
        #Saving trial based results
        with open('./results/kfold_trial/trial_based/'+type(reg_sac[1]).__name__+'_y_pred_'+config+'_'+str(fold)+'.npy', 'wb') as f:
            np.save(f, y_pred)
        with open('./results/kfold_trial/trial_based/'+type(reg_sac[1]).__name__+'_y_test_'+config+'_'+str(fold)+'.npy', 'wb') as f:
            np.save(f, y_test)
...
for models, configuration in  [ (models_regression, 'all_features'), 
    ...
    for model in models:
        cv_summary = get_results_kfold(X_fix, ids_f, yf, stim_f, X_sac, ids_s, ys, stim_s, k=5, model=model, config=configuration)

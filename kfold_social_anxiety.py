...
def evaluate(reg_fix, reg_sac, X_fix_test, y_f_test, stim_f_test, sub_f_test, X_sac_test, y_s_test, stim_s_test, sub_s_test, fold, config):
    ...
    if type(reg_fix) ==  GPy.models.SparseGPRegression:
        ...
        #Saving datapoint based results
        with open('./results/parallel/datapoint_based/'+type(reg_fix[1]).__name__+'_'+config+'_'+'_ppred_fix_'+str(fold)+'.npy', 'wb') as f:
            np.save(f, ppred_fix)
    ...
    if type(reg_sac) ==  GPy.models.SparseGPRegression:
        ...
        #Saving datapoint based results
        with open('./results/parallel/datapoint_based/'+type(reg_sac[1]).__name__+'_'+config+'_'+'_ppred_fix_'+str(fold)+'.npy', 'wb') as f:
            np.save(f, ppred_sac)
